...
{
    public partial class ViewChequeDisbursesReport : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (Session["vwChequeDisbursementsReport"] != null)
                {
                    DataTable dt = (DataTable)Session["vwChequeDisbursementsReport"];
                   dt.Rows[0]["AmountText"] = new NumberToWords.NumberToWords().GetAmountWord(decimal.Parse(dt.Rows[0]["Debit"].ToString()), NumberToWords.NumberToWords.ValueTypes.Money);
                    ...
                    string rptFullPath = Server.MapPath("rptChequeDisbursements.rpt");
            ...
            {
                Session.Remove("vwChequeDisbursementsReport");
            }
        }
    }
}